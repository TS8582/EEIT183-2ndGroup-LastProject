...
public interface PropSellOrderRepository extends JpaRepository<PropSellOrder, Integer> {
	...
	//根據gameId及status=0找賣單	
	@Query("SELECT o FROM PropSellOrder o WHERE o.memberPropInventory.props.game.gameId = :gameId AND o.orderStatus = 0" )
    List<PropSellOrder> findAllByGameIdAndStatus0(@Param("gameId") int gameId);
